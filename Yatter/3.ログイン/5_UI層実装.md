...
公式から「`SingleLiveEvent`クラスは`LiveData`の利点がなくなっている」と言われていますが、業界全体的にもまだベストプラクティス的な代案が見つかっていないため利用されています。
実装は既に済んでいますので興味ある方は実装を見てみてください。
...
プレビューを確認すると亜p理上部にAppBarとログインタイトルが表示されます。
