...
実装に入る前に、Jetpack Composeでの画面遷移の方法を学びましょう。Jetpack Composeでは画面遷移に[navigation-compose](https://developer.android.com/develop/ui/compose/navigation?hl=ja)を使うことが多いです。

`navigation-compose` で画面遷移を実装するには、まず `NavHost` の定義をしてrouteの設定をする必要があります。

`NavHost` はComposable関数で、引数に `navController`, `startDestination`, `builder` を受け取ります。 `startDestination` は初期状態で表示するrouteです。
...
Destinationを実装したので、ログイン画面のUI層実装で保留にした画面遷移の実装を完成させましょう。`LoginViewModel`の`onClickLogin`でログインが成功した際に`_destination`のStateFlowを`PublicTimelineDestination`で更新します。ログイン後にはパブリックタイムラインから戻るボタンでログイン画面に戻ってしまわないように、オプションとして`launchSingleTop = true`を指定しています。
