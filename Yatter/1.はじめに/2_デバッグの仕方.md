...
Android Studioは、Androidアプリ開発のための統合開発環境（IDE）であり、強力なデバッグツール群を提供しています。これらのツールを活用することで、コードの実行をステップごとに追跡したり、変数の値を確認したり、実行時のエラーを特定したりすることが容易になります。代表的な機能には、Logcat、ブレイクポイント、Layout Inspectorなどがあります。これらの機能を使いこなすことで、開発効率を大幅に向上させることができます。
...
デバッグモードでは、ブレイクポイントでの停止、変数の監視、ステップ実行などの機能が利用可能になります。
...
## BreakPoint
ブレイクポイントは、コードの特定の行でプログラムの実行を一時停止させるためのマーカーのようなものです。デバッグモード（Run > Debug 'app'）でアプリを実行すると、処理実行がブレイクポイントに到達した時点で一時停止します。

### ブレイクポイントの設定方法
Android Studioでは、一時停止したいコードの行番号をクリックするだけで簡単にブレイクポイントを設定・解除できます。

![ブレイクポイントの設定](../image/1/add_break_point.png)

### ブレイクポイントの種類
- **行ブレイクポイント**: 特定の行で停止する基本的なブレイクポイント
- **メソッドブレイクポイント**: 特定のメソッドが呼び出されたときに停止
- **例外ブレイクポイント**: 特定の例外が発生したときに停止
- **フィールドウォッチポイント**: 特定のフィールドの値が変更されたときに停止
- **条件付きブレイクポイント**: 特定の条件が満たされたときのみ停止

### 条件付きブレイクポイントの設定
1. 通常通りブレイクポイントを設定
2. 「Condition」フィールドに条件式を入力（例: `userId == 123`）

### 停止時の値の確認方法
実行がブレイクポイントで停止すると、Android Studioの下部に**Debugウィンドウ**が表示されます。
*   **Variables**: 現在のスコープにある変数とその値の一覧が表示されます。オブジェクトを展開して、そのプロパティやフィールドの詳細を確認することも可能です。
*   **Watches**: 特定の変数や式を常に監視できます。「+」ボタンをクリックして監視したい変数名や式を追加します。
*   **Frames**: コールスタック（メソッド呼び出しの階層）が表示されます。各フレームをクリックすると、そのメソッド呼び出し時点のコンテキストに切り替わります。

### ステップ実行
実行が停止した状態から、以下のボタンを使ってコードを一行ずつ進めることができます：

- **Step Over**: 現在の行を実行し、次の行に進む（メソッド呼び出しの中には入らない）
- **Step Into**: メソッド呼び出しの中に入る
- **Step Out**: 現在のメソッドから抜ける
- **Resume Program**: 次のブレイクポイントまで実行を再開

### Evaluate Expression
実行が停止している状態で、任意の式を評価することができます：

1. `Alt+F8`キーを押すか、右クリックメニューから「Evaluate Expression」を選択
2. 評価したい式を入力（例: `user.getFullName()` や `list.size`）
...
Shiftを2回押して「Toggle layout inspector」を選択して「ON」にすることで起動します。接続されているデバイスまたはエミュレータで実行中のアプリのプロセスを選択すると、Layout Inspectorウィンドウが開きます。
...
最も一般的な例外の一つで、nullオブジェクトのメソッドやプロパティにアクセスしようとした場合に発生します。Composeでも発生する可能性があります。
...
**解決方法**:
- Logcatでスタックトレースを確認し、どの行でエラーが発生しているかを特定
- 該当する変数が初期化されているか確認
- Kotlinの場合、Null安全演算子（`?.`）やElvis演算子（`?:`）を使用
- Composeの状態管理で、初期値が適切に設定されているか確認
- `remember { mutableStateOf(null) }` のような初期値がnullの状態を使用する場合は型を明示する（例: `remember { mutableStateOf<User?>(null) }`）

### IllegalStateException
Composeでは、コンポーネントが予期しない状態にある場合に発生することがあります。

**エラーメッセージの例**:
```
java.lang.IllegalStateException: Calling LaunchedEffect in composition without being installed in the composition
```

**解決方法**:
- Composeのライフサイクルを理解し、適切なタイミングで処理を行う
- `LaunchedEffect`や`DisposableEffect`などの副作用を正しく使用する
- 状態変更前に適切な条件チェックを追加
- コンポジションの外部で状態を変更しないようにする
...
**解決方法**:
- 重い処理はバックグラウンドスレッドで実行（Coroutines、RxJava、Threadなど）
...
### 2. ログの活用
- 重要なポイントにログを配置する
- 開発中は詳細なログを出力し、本番環境ではログ出力しすぎないようにする
...
- 例えば、Logcatでエラーを特定し、ブレイクポイントで詳細を調査し、Profilerでパフォーマンス問題を確認するなど

## その他の便利なデバッグツール

### Android Profiler
メモリ使用量、CPU使用率、ネットワークアクティビティなどを監視するためのツールです。
...
- **Network Profiler**: ネットワークリクエストの監視
- **Energy Profiler**: バッテリー消費の分析

### Database Inspector
Room、SQLiteなどのデータベースの内容をリアルタイムで確認・編集できるツールです。

1. Android Studioのメニューから `View > Tool Windows > App Inspection` を選択
2. 「Database Inspector」タブを選択
...
### Wireless Debugging
Android 11以降では、USBケーブルなしでデバッグが可能になりました。

設定方法：
1. デバイスの開発者オプションで「ワイヤレスデバッグ」を有効化
2. Android Studioの「Device Manager」から「Pair using Wi-Fi」を選択
3. デバイスに表示されるペアリングコードを入力
