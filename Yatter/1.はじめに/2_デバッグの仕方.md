...
### デバッグモードでの実行
通常の実行（Run）とは異なり、デバッグモードで実行することで、より詳細な情報を得ることができます。

1. Android Studioのツールバーにある「Debug」ボタン（虫のアイコン）をクリックする
2. または、`Run > Debug 'app'`メニューを選択する
...
### Logcatの表示方法
1. Android Studioの下部にある「Logcat」アイコンをクリックする
2. または、`View > Tool Windows > Logcat`メニューを選択する

### Logcatの効果的な使い方
- **フィルタリング**: 上部の検索ボックスを使用して、特定のタグやメッセージでフィルタリングできます
- **ログレベルの選択**: Verbose、Debug、Info、Warn、Error、Assertの各レベルでフィルタリングできます
- **プロセスの選択**: 複数のアプリが実行されている場合、特定のアプリのログのみを表示できます
- **正規表現の使用**: 複雑なフィルタリングには正規表現を使用できます
- **フィルタリングせずに検索**: 「⌘ + F」で検索できます

### 実践的なフィルタリング例
- アプリのパッケージ名: `package:com.dmm.bootcamp.yatter2025`
  - Android Studioで開いているアプリのみ: `package:mine `
- 特定のタグ: `tag:Yatter`
- エラーのみ: `level:error`
...
- **メソッドの開始/終了**: 複雑なメソッドの開始と終了をログに記録すると、処理の流れを追いやすくなる
...
D/OkHttp: 
D/OkHttp: {"username":"user123","password":"********"}
...
- **メソッドブレークポイント**: 特定のメソッドが呼び出されたときに停止
- **例外ブレークポイント**: 特定の例外が発生したときに停止
- **フィールドウォッチポイント**: 特定のフィールドの値が変更されたときに停止
- **条件付きブレークポイント**: 特定の条件が満たされたときのみ停止

### 条件付きブレークポイントの設定
1. 通常通りブレークポイントを設定
2. ブレークポイントを右クリックして「More」を選択
3. 「Condition」フィールドに条件式を入力（例: `userId == 123`）
...
- **Step Over (F8)**: 現在の行を実行し、次の行に進む（メソッド呼び出しの中には入らない）
- **Step Into (F7)**: メソッド呼び出しの中に入る
- **Step Out (Shift+F8)**: 現在のメソッドから抜ける
- **Resume Program (F9)**: 次のブレークポイントまで実行を再開
...
- **Live Updates**: リアルタイムでUIの変更を反映（Live Updateモードが有効な場合）
...
### 2. ログの活用
- 重要なポイントには適切なレベルのログを配置する
- 開発中は詳細なログを出力し、本番環境では必要最小限にする
...
### 4. チームでのデバッグ
- 行き詰まったら、他の開発者に相談する（ラバーダック・デバッグ）
- 問題と解決策を文書化して共有する
...
最後に、デバッグは忍耐と論理的思考が求められる作業です。一度にすべてのテクニックを習得する必要はなく、実際の開発を通じて少しずつスキルを磨いていきましょう。
